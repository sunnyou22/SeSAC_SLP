...
class CustomAlertView: BaseView {
    ...
    lazy var alertLbl: UILabel = {
        let view = UILabel()
        return setattributeText(view: view, text: "스터디를 요청할게요!\n요청이 수락되면 30분 후에 리뷰를 남길 수 있어요", location: 0, length: 11, baseColor: .setGray(color: .gray7), pointColor: .setBaseColor(color: .black))
    }()
    
    lazy var buttonStackView: UIStackView = {
        let view = UIStackView(arrangedSubviews: [okButton, noButton])
    ...
    lazy var totalStackView: UIStackView = {
        let view = UIStackView(arrangedSubviews: [buttonStackView, alertLbl])
        view.axis = .vertical
